/* Editorial Discovery Console: dark app shell, image-led catalogue, coral activation, and local-first interactions. */
import { useMemo, useState } from "react";
import {
  BadgeCheck, Bookmark, BookmarkCheck, ChevronLeft, ChevronRight, CircleHelp, Compass, Crown, Globe2,
  HeartHandshake, House, Image, LockKeyhole, Menu, MessageCircle, Play, Plus, Search, Sparkles, Trophy,
...
function CharacterCard({ character, saved, onSave, onOpen }: { character: Character; saved: boolean; onSave: () => void; onOpen: () => void }) {
  return <article className={`character-card ${saved ? "is-saved" : ""}`}>
    ...
    <button className={`save-button ${saved ? "saved" : ""}`} onClick={onSave} aria-label={saved ? `Remove ${character.name} from saved` : `Save ${character.name}`}>
    ...
    <div className="card-copy">
      <button className="profile-open" onClick={onOpen}><h3 className="card-title">{character.name} <span className="card-age">{character.age}</span></h3></button>
      <p className="card-description">{character.description}</p>
      {character.play && <button className="play-chip" onClick={onOpen} aria-label={`Play ${character.name}'s story`}><Play size={10} fill="currentColor" /> Play</button>}
    </div>
  </article>;
}
...
function ProfileDrawer({ character, saved, onClose, onSave, onChat }: { character: Character; saved: boolean; onClose: () => void; onSave: () => void; onChat: () => void }) {
  return <div className="profile-layer" role="dialog" aria-modal="true" aria-label={`${character.name} profile`}>
    <button className="profile-backdrop" onClick={onClose} aria-label="Close profile" />
    <aside className="profile-drawer">
      <div className="profile-visual"><img src={character.image} alt="" /><div className="profile-visual-shade" /><button className="drawer-close" onClick={onClose} aria-label="Close profile"><X size={20} /></button>{character.tag && <span className="status-chip drawer-tag">{character.tag === "Series" ? <Play size={9} fill="currentColor" /> : <Sparkles size={10} />} {character.tag}</span>}</div>
      <div className="drawer-content"><p className="drawer-eyebrow">CHARACTER SPOTLIGHT</p><h2>{character.name} <span>{character.age}</span></h2><p className="drawer-description">{character.description} This profile is ready for a light, conversational discovery flow.</p><div className="drawer-meta"><span><Sparkles size={14} /> {character.category}</span><span><Globe2 size={14} /> Available now</span></div><div className="drawer-actions"><button className="drawer-secondary" onClick={onSave}>{saved ? <BookmarkCheck size={17} fill="currentColor" /> : <Bookmark size={17} />}{saved ? "Saved" : "Save profile"}</button><button className="drawer-primary" onClick={onChat}><MessageCircle size={17} />Start chat</button></div></div>
    </aside>
  </div>;
}
...
export default function Home() {
  ...
  const visibleCharacters = useMemo(() => {
    ...
    const filtered = characters.filter((item) => activeFilter === "All" || (activeFilter === "Saved" ? savedProfiles.includes(item.name) : item.category === activeFilter));
    return normalized ? filtered.filter((item) => `${item.name} ${item.description}`.toLowerCase().includes(normalized)) : filtered;
  }, [activeFilter, query, savedProfiles]);
  ...
  const openCharacter = (character: Character) => setSelectedCharacter(character);
  ...
  const selectNav = (label: string) => {
    setActiveSection(label); setSidebarOpen(false);
    if (label === "Collection") goToCatalogue("Saved");
    if (label === "Discover" || label === "Home") goToCatalogue("All");
    if (label === "Chat") showActivity("Choose a profile to begin a conversation.");
  };

  return <div className="app-shell">
    <header className="topbar">
      <button className="icon-button" onClick={() => setSidebarOpen(!sidebarOpen)} aria-label="Toggle menu"><Menu size={27} /></button>
      <a className="brand" href="#discover" onClick={() => goToCatalogue("All")} aria-label="Discover home"><img src="/manus-storage/discover-symbol_9d1479e3.png" alt="" />discover<span className="brand-accent">.</span></a>
      <nav className="top-nav" aria-label="Audience selection"><button className="active"><UsersRound size={15} /> Discover</button><button onClick={() => goToCatalogue("New")}><BadgeCheck size={15} /> Explore</button><button onClick={() => goToCatalogue("Spotlight")}><UserRound size={15} /> Connect</button></nav>
      <div className="top-actions"><button className="button button-primary" onClick={() => showActivity("Account creation is ready for connection in the next build.")}><Plus size={14} /><span className="create-label">Create account</span></button><button className="button button-ghost" onClick={() => showActivity("Login is ready for connection in the next build.")}>Log in</button></div>
    </header>
